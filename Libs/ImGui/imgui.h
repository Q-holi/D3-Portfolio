...
namespace ImGui
{
    ...
    IMGUI_API void          PushID(std::string* str_id);                                     // push string into the ID stack (will hash string).
    ...
    IMGUI_API bool          BeginCombo(const char* label, std::string* preview_value, ImGuiComboFlags flags = 0);
    IMGUI_API void          EndCombo(); // only call EndCombo() if BeginCombo() returns true!
    IMGUI_API bool          Combo(const char* label, int* current_item, std::string* items, int items_count, int popup_max_height_in_items = -1);

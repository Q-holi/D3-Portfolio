...
bool ImGui::BeginCombo(const char* label, std::string* preview_value, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
...
bool ImGui::Combo(const char* label, int* current_item, std::string* items, int items_count, int popup_max_height_in_items)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items);
    return value_changed;
}
